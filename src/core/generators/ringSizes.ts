...
export function sizeToCircumference(system: SizeSystem, size: number): number {
  switch (system) {
    ...
    case 'UK': {
      const i = Math.min(Math.max(size, 0), UK_CIRC.length - 1)
      const lo = Math.floor(i)
      const hi = Math.min(lo + 1, UK_CIRC.length - 1)
      return UK_CIRC[lo] + (UK_CIRC[hi] - UK_CIRC[lo]) * (i - lo)
    }
  }
}
...
export function circumferenceToSize(system: SizeSystem, c: number): number {
  switch (system) {
    ...
    case 'UK': {
      if (c <= UK_CIRC[0]) return 0
      const last = UK_CIRC.length - 1
      if (c >= UK_CIRC[last]) return last
      let i = 0
...
/** UK letter index → display label, rounded to the nearest half ("N½"). */
export function ukLabel(index: number): string {
  const snapped = Math.round(index * 2) / 2
  const letter = String.fromCharCode(65 + Math.floor(snapped))
  return snapped % 1 === 0.5 ? `${letter}½` : letter
}
